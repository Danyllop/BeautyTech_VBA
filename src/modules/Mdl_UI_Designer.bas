...
'
' -------------------------------------------------------------------------
' CONTROLADOR MESTRE: Chama todas as configurações de uma vez
' -------------------------------------------------------------------------
Public Sub ConfigurarIconesDoSistema(ByVal Frm As Object, ByVal ColStorage As Collection)
    ' Chama as rotinas individuais passando a Coleção para guardar a classe
...
Public Sub ConfigIcoSair(ByVal Frm As Object, ByVal ColStorage As Collection)
    ...
    With Frm.IcoSair
        ...
        ' 2. Configuração de Comportamento (Classe Hover)
        Set cls = New clsIconeHover
        ' Define a cor Vermelho Suave (RGB 255, 80, 80)
        cls.Inicializar Frm.IcoSair, RGB(255, 80, 80)
        
        ' Guarda na coleção para manter o efeito funcionando
        ColStorage.Add cls
    End With
End Sub

' -------------------------------------------------------------------------
' CONFIGURAÇÃO: Ícone Modo Dev (Verde Tech)
' -------------------------------------------------------------------------
...
' -------------------------------------------------------------------------
' CONFIGURAÇÃO: Ícone Usuários (Padrão Branco)
' -------------------------------------------------------------------------
Public Sub ConfigIcoUsuarios(ByVal Frm As Object, ByVal ColStorage As Collection)
    ...
    With Frm.IcoUsuarios
        ...
        ' 2. Configuração de Comportamento
        Set cls = New clsIconeHover
        ' Sem cor definida = usa o padrão Branco (definido na classe)
...
Public Sub RedimensionarMenuLateral(ByVal Frm As Object)
    ...
    With Frm.ImgLogo
        ...
        .PictureSizeMode = 3 ' fmPictureSizeModeZoom (Garante proporção)
    End With
...
Public Sub ConfigIcoDash(ByVal Frm As Object)
    
    With Frm.MicDashboard
